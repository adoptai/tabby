...
24. **Kind pod MTU defaults to 65535** — Breaks TLS handshakes to external sites whose certs produce large packets (GitHub, LinkedIn CDN). The pod MTU is **derived, not configured**: Docker Desktop's bridge is 65535, kind's network inherits it, and kindnetd copies the node's `eth0` MTU into `/etc/cni/net.d/10-kindnet.conflist` on **every start** — so patching that file is always reverted. Two fixes, both local-Kind only:
    - `make kind-create` gives the cluster its own Docker network at MTU 1500 (`KIND_NETWORK`, via `KIND_EXPERIMENTAL_DOCKER_NETWORK`). Bridge, node and pod MTU all agree; the problem never exists.
    - `make kind-fix-mtu` (existing clusters) writes `05-tabby-mtu.conflist` — a copy of kindnet's config at MTU 1500. containerd loads the lexicographically **first** config, so ours wins and kindnetd never touches a file that isn't its own. Survives kindnet restarts. Re-run only after a kindnet **image upgrade** (the copy is a snapshot).

    Never lower the node's own `eth0` to fix this: with the bridge still at 65535 the node drops frames the bridge emits, wedging `kubectl port-forward` ("error creating error stream: Timeout occurred") and killing the API/VNC tunnels. Existing pods keep their old MTU — `kubectl rollout restart deploy -n browser-hitl` to pick it up.
