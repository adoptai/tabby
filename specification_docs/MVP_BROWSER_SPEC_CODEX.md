...
- **Headers:** Capture via `page.on('response')` event listener (passive, not `page.route()`). Use `response.allHeaders()` (not `response.headers()`, which filters security-related headers). Filter captured headers against `export_policy.header_allowlist`. Register listener before triggering login actions.
